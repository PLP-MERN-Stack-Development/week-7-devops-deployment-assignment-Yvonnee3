...
const TaskManagerPage = () => {
  // useLocalStorage for persisting tasks
  const [tasks, setTasks] = useLocalStorage('tasks', []);
  // useState for new task input
  const [newTask, setNewTask] = useState('');
  // useState for task filtering
  ...
  return (
    <div className="min-h-[calc(100vh-180px)] p-4 flex flex-col items-center">
      ...
      <Card className="w-full max-w-md">
        ...
        <div className="flex justify-center space-x-2 mb-4 flex-wrap gap-2"> {/* Added flex-wrap for small screens */}
          <Button
        ...
        {filteredTasks.length === 0 ? (
          ...
          <ul className="space-y-3">
            {filteredTasks.map((task) => (
              ...
              >
                <span
                  className={`flex-grow cursor-pointer text-gray-900 dark:text-gray-100 ${
                    task.completed ? 'line-through text-gray-500 dark:text-gray-400 italic' : ''
                  }`}
